...
**Backlog (deferidos com motivo):** `TripSummary`/`repasseSugerido` mostram negativo transitório entre digitar Total e sair do campo (ResultSummary/card já mostram "—"); `RegrasTab` com duas versões futuras mostra só a primeira; `ReceberModal` alerta "marque Registrar mesmo assim" persiste até reenviar; `Page` não limpa `document.title` ao desmontar (login herda); `ConviteService.RevogarAsync` sem linha de auditoria (mesmo padrão do reenviar); `DefinirValorAsync` aceita valor em repasse `cancelado`; `th` do DataTable esconde a sombra de scroll no cabeçalho; `usePessoa` `useQuery` duplicado até `useFormularioCadastro` expor o erro; `anexos.ts` `new URL()` fora do try; sombra de scroll usa navy 12 % (token novo `--table-scroll-shadow`); A24 fase `sem_reserva` continua derivada por view (só rótulo mudou); M01 aba "canceladas com recebimento" na conciliação; A07 header "Recebida" com divergência (spec §6.1).
