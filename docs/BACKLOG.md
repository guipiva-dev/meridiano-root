...
Ressalvas: skew de relógio API×Postgres > 1 s faz o auto-login pós-reset cair uma vez (401, usuário loga de novo) — se acontecer, trocar por claim `senha_alterada_em` no cookie comparada com o banco (sem relógio, sem coluna nova); timeout HTTP no front mostra "Sem conexão". Segunda rodada dos reviews (2026-09-11): sem código novo além de `act()` em `BuscaGlobal.test.tsx`; cifrar `data_protection_key` (Key Vault/certificado), fallback de `AbortSignal.timeout`, bundle 640 kB e logging do ErrorBoundary ficam para quando houver infra/medição.
Rodadas 3–4 (2026-09-11): backend `e52d035` — `ci.yml` grava labels `org.opencontainers.image.revision` e `meridiano.frontend.revision` na imagem (rastreabilidade sem fixar o front em SHA); `docs/deploy.md` corrigido para 18 migrations e cabeçalho "smoke local feito, nuvem não". Rejeitados: fixar front por SHA (atrito, mesmo dono), restore automatizado (drill manual é passo do runbook §10), limpeza de `data_protection_key` (~4 chaves/ano), teste de presença da 0018 (`MigrationsTests` já cobre). Hooks do harness passaram a resolver por `$CLAUDE_PROJECT_DIR` (root `0cc36e8`): hook relativo falhava com `MODULE_NOT_FOUND` após `cd` para `backend/`/`frontend/`.
Pendente de push: backend `b57ab27..e52d035`, frontend `f6f0ceb..7de4f73`, root `2581e18..HEAD`.
...
- Banco do compose ficou **sem grants** para `meridiano_api` após o drill de restore (`pg_restore --no-privileges`): login devolvia 403 `sem_acesso` (`42501: permission denied for table log_acesso`). Só `data_protection_key` (criada depois, via default privileges) tinha grant. Fix = passo 4 do `docs/deploy.md` §10; reaplicado à mão 2026-09-11.
- Acesso externo ao stack local para o teste humano (§7/§12): Cloudflare Tunnel `gui-casa` (serviço Windows, token remoto) → `http://localhost:5000`, hostname `https://mediterraneo-app.bspdv.com.br/` (cadastrado com typo; `meridiano-app` não existe). Cookie `SameAsRequest` em Development + `UseForwardedHeaders` bastam.
...
### Homologação 2026-09-12 (rodada 2) → correções 2026-09-12 (plano `docs/superpowers/plans/2026-09-12-correcoes-homologacao-2.md`)
Auditoria externa (`Auditoria_Homologacao_Meridiano_2026-09-12.docx`, 27 achados) executada em 11/09 ~21h–23h BRT — **antes** do merge da rodada 1. Triagem contra `main`: 8 já fechados (ALT-03/05/06/08, MED-01/05/08, ALT-15 despesa/fornecedor/colaborador); 4 por design (ALT-11 taxa independe do custo §4.2; ALT-14 repasse é digitado, nunca recalculado §5; MEL-01 contrato de design não usa "Salvando…"; BLQ-01 502 do tunnel local); 15 corrigidos. Branch `fix/homologacao-2` mergeada em `main` (backend `dd14855`, frontend `1a7a6a7`, sem push). 10 tasks + 2 fix waves finais; 305 API + 38 domínio, 679 Vitest. Sem migration.

Fechados no código: reserva sem fornecedor bloqueada no front com erro no card (ALT-01); documento exige `numero` (ALT-02 — a rodada 1 tinha chamado de falso positivo; era real); telefone/whatsapp só dígitos, `+DDI` E.164 10–15, códigos `telefone_invalido`/`whatsapp_invalido`/`telefone_emergencia_invalido` (ALT-04/07); site http(s) (ALT-07); badge "Comissão: Recebida" com tooltip (ALT-09); agente só perfil dono/agente em transferir **e** criar/editar viagem (ALT-10); título/códigos com `min-width:0; overflow-wrap:anywhere` (ALT-12); Fechar navega para a viagem salva (ALT-13); excluir grupo (UI) e cliente (`DELETE /clientes/{id}`, 422 `cliente_com_vinculos`) (ALT-15); busca da lista com `flex:none` nos irmãos (MED-02); limites de texto na API (`Guardas.Texto`, 422 `texto_longo` + extensão `campo`) e `maxLength` no front: destino 120, localizador 40, nome 150, cidade 80, título 150, descrição 200, observações 2000 (MED-03); CSV via fetch com "Gerando…" e Alert (MED-04, relatórios e auditoria); filtro "Valores de reserva" inclui INSERT (MED-06); contadores "—" enquanto carrega (MED-07); "Ativas N · Total M" (MED-09); tooltips em Comissão/Taxa/Fluxo (MEL-02). Exibição de telefone com máscara (`formatarTelefone`) nas listas e forms.
...
### Homologação 2026-09-12 (rodada 3, Playwright, 67 achados) → correções 2026-09-13 (plano `docs/superpowers/plans/2026-09-13-correcoes-homologacao-3.md`, ledger `.superpowers/sdd/2026-09-13-correcoes-homologacao-3/progress.md`)
Relatório: `docs/piloto/auditoria-homologacao-3-2026-09-12.md`. Branch `fix/homologacao-3` mergeada em `main` e enviada ao origin (backend `86f0b54`, frontend `fa33f4f`), 13 tasks em 2 ondas + onda de correção final; 39 domínio + 329 API, 794 Vitest.

**Corrigido:** taxa ≤ total, venda nula/0 = total (API e pré-preenchimento no form), esperado da operadora negativo bloqueado (create/edit/remarcar com valor), NFSe emitida exige número + data ≤ hoje, CPF sempre serializado (`ClienteComDocumentoDto`) e campo por permissão, CNPJ único, agenda só viagem internacional, `fornecedor.ver` (vendedor externo fora), repasse do vendedor no DTO da própria viagem, cabeçalhos de segurança + CSP (`connect-src` com o storage), crédito/reembolso ≤ venda, `DELETE /usuarios/{id}/convite`, repasse `cancelado` em viagem cancelada (migration 0023 + saneamento `valor_cliente=0` + `vw_resultado_viagem`), `repassesPagos` no resultado operacional, janelas 1–31 sem lacuna + versão do mesmo dia substitui + `vigente`, lista de viagens cabe em 1366 (1ª coluna absorve), grids/tabs/subnav responsivos, `Ida:` no filtro, plural, feedback de erro em todo dialog (5xx/rede/upload/excedente), Equipe (Editar em toda linha, cancelar convite, toasts, auto-inativação bloqueada na UI), rótulos "recebido" distintos, excedente sinalizado, "Encerrar divergência" visível, despesa de viagem sem recorrência, aba Financeiro da viagem separa recebidas/divergentes (sem "Receber"), crédito por fornecedor explicado, aviso de estorno ao cancelar reserva com comissão recebida, "Sem reserva ativa", `document.title` por página, `rem` = 16 px (root corrigido; contrato 12/13/14/24), rótulos da auditoria, copies.

**B01 fechado na operação (13/09):** MinIO exposto no tunnel como `mediterraneo-storage.bspdv.com.br`; `Armazenamento__Endpoint` como variável de usuário do Windows; upload e download verificados pelo host público. **Reteste 13/09** (`docs/piloto/reteste-homologacao-3-resultado.md`): 10/10 PASS; sobra de plural corrigida (frontend `fa33f4f`).

**Backlog técnico fechado (2026-09-13, plano `docs/superpowers/plans/2026-09-13-backlog-tecnico-rodada-3.md`, branch `fix/backlog-r3` mergeada: backend `f236076`, frontend `8cc1d43`; 334 API + 39 domínio, 827 Vitest):** venda vazia fora dos totais/repasse sugerido (`somarReservas.incompleta`); todas as versões futuras da regra listadas; alerta do excedente some ao marcar; `useFormularioCadastro.erroCarga` (sem `useQuery` duplicado); sombra de rolagem por `useScrollShadow` cobrindo o cabeçalho; `enviarArquivo` trata URL inválida; auditoria ao reenviar/revogar convite; `DefinirValor` recusa repasse `cancelado`. **Pedidos do dono na mesma onda:** CPF e data de nascimento obrigatórios no cliente (POST/PUT; `cpf_obrigatorio`/`data_nascimento_obrigatoria`; "+ pessoa" ganhou Nascimento); chip do passageiro mostra CPF formatado e nascimento (busca devolve `dataNascimento`); hover dos chips com contraste (`Chip` compartilhado); forma de pagamento **Dinheiro** na reserva (**migration 0024**).

**Backlog restante (motivo):** `RevogarAsync` grava `de: "pendente"` mesmo com convite expirado (rótulo da auditoria; só cosmético); cliente legado sem CPF editado por perfil sem `cliente.ver_documento` não consegue completar o CPF (por design: só quem vê documento edita; o Dono/Agente completam); `TripSummary` na visão do agente mostra "Receita das reservas" parcial sem aviso quando falta venda (Resultado já mostra "—"); A24 fase `sem_reserva` derivada por view (só rótulo); M01 aba "canceladas com recebimento"; A07 header "Recebida" com divergência (spec §6.1).

**Decisão de design/produto (não fazer sem decisão):** 3 CTAs "Nova viagem"; laranja × azul; validação sequencial do cadastro; Enter no Remarcar; motivo ao corrigir movimento; AC02 (botão sem nome — não reproduzido).
...
### Benchmark Pax Pro 2026-09-14 (`docs/benchmark/paxpro-2026-09-13.md`, branch `feat/benchmark-paxpro`)
- **Aplicado:** testes de regressão financeira (fuso, data futura, estorno com motivo, cancelamento preserva movimentos) + `despesa.pago_em` futuro → 422 `data_invalida`; duplicar reserva (com aviso de localizador duplicado); botão "Enviar mensagem" (wa.me + atendimento registrado) nas pendências check-in/pós-viagem, via `mensagemTipo` no DTO; pendência derivada `pendencia:voucher:<reserva>` (reserva emitida sem anexo a ≤ 7 dias do embarque).
- **Mergeado e no origin 2026-09-14:** backend `26c83b4`, frontend `0b405d7` (subiu junto a rodada 4, que não tinha sido enviada).
- **Lançamento (branch frontend `feat/lancamento-paxpro`):** comissão em R$ ou %; idade do passageiro na data de ida com faixa bebê/criança; aviso não bloqueante de data de serviço fora da viagem no modal de serviço (rótulos por tipo). Deferido: indicador do mesmo aviso na lista de serviços; modo % não persiste ao recolher o card. Validar no T5: volta do aéreo encadeada, passageiros por reserva.
- **Deferido:** textos das mensagens fixos no front (não há `agencia.config` editável); dados de serviço/moeda não entram na duplicação (não estão no DTO da reserva).
- **Decisões pendentes do dono:**
  1. **Motivo ao editar reserva conciliada** — §8 exige; código só exige em competência fechada. Teste pronto em `RegressoesBenchmarkTests` com `Skip`; implementação = guarda no fluxo de edição de reserva em `Modules/Viagens`.
  2. **Texto de ajuda no modal de anexo** — modal vem com "Vínculo: Viagem" por padrão; voucher anexado na viagem não resolve a pendência de voucher (job só olha `anexo.reserva_id`). Proposta: dica "para voucher, escolha a reserva". Não contar anexo da viagem (qualquer anexo apagaria a pendência de todas as reservas).
  3. **Vencimento do pagamento à operadora** (`cliente_paga_agencia`) — reabre §9 "fora da v1". Requisito REQ-03 no benchmark: campo `reserva.vencimento_fornecedor` + situação derivada + pendência automática.
  4. **Vencimentos/parcelas do que o cliente deve à agência** — contraria "o sistema não parcela". Validar com as 3 viagens do T5; se nenhuma tiver cliente pagando a agência em mais de uma vez, fica aqui. Requisito REQ-02 no benchmark.
  5. **Previsão de 30 dias no dashboard** (entra/sai/atrasado, sem saldo bancário) — só faz sentido depois de 3 e/ou 4. Requisito REQ-05 no benchmark.

### Financeiro da reserva simplificado 2026-09-14 (plano `docs/superpowers/plans/2026-09-14-financeiro-reserva-simplificado.md`, ledger `.superpowers/sdd/2026-09-14-financeiro-reserva-simplificado/progress.md`, branch `feat/financeiro-simplificado` backend + frontend, **sem merge**)
- **Rulings do dono:** fornecedor sempre já pago (Fluxo sai da tela); comissão % e R$ lado a lado + comissão média na viagem; comissão do vendedor em % sobre comissão + RAV (migration **0026** `repasse.percentual`, recalculado em `ReavaliarRepasseAsync` enquanto não pago); RAV único calculado (cobrado − total), sem RAV da operadora e sem modo na tela; taxa de serviço visível; resumo da reserva como conta; rótulos únicos nas faixas. Decisões 3 e 4 da seção Benchmark ficam sem efeito (fornecedor já pago).
- **Deferido (revisões):** teste de troca de vendedor com %; `numeric(5,2)` arredonda 10,555 sem 422; formulário não indica RAV da operadora legado > 0 (só o resumo); testes A03 saem do Total por clique; `RemarcarModal` ainda valida por `via_operadora`; avisos react-refresh por helpers exportados de `TripSummary.tsx`; texto de % do vendedor não reseta ao recarregar; regex frouxa em `ResumoTab.test`; F5 só zera repasse quando o vendedor está na lista carregada; PUT da viagem sempre regrava a linha do repasse (xmin) — pré-existente.
- **Divergência visível:** faixa do formulário exclui reserva cancelada com `comissao_mantida` da "Receita da agência"; faixa do detalhe (`vw_resultado_viagem`) inclui. Pré-existente; alinhar.
- **Mergeado em `develop` + push 2026-09-14** (backend `cfb35a4`, frontend `788d02a` + fix e2e). Playwright `nova-viagem` cronometrado: **5,6 s @1280 / 5,9 s @1440** (antes 6,2 / 7,7); helper agora digita no "Total cobrado do cliente" já focado após o Tab. "Aviso de viagem semelhante" corrigido: o spec esperava o título fraco, mas datas sobrepostas mostram o aviso forte (§ regras) — não era dado de homologação; frontend `01e3331`. Piloto atualizado 2026-09-14 16:53 (build develop, migration 0026 aplicada).

### Redesign viagem — Fase 1 (2026-09-14) (plano `docs/superpowers/plans/2026-09-14-redesign-viagem-fase1.md`, spec `docs/superpowers/specs/2026-09-14-redesign-viagem-fase1-design.md`, ledger `.superpowers/sdd/2026-09-14-redesign-viagem-fase1/progress.md`, branch `feat/redesign-viagem-f1` backend + frontend, **sem merge**)
- **Rulings do dono:** design descongelado para este redesign (`docs/design-system-contrato.md` §4.5); laranja (`business`) passa a significar "ação principal da tela", não "ação de negócio" — Nova viagem / Edição usa em "Salvar viagem" ("+ Adicionar reserva" vira `secondary`); **viagem só salva completa** — criar ou editar exige data de ida, data de volta e ao menos uma reserva de qualquer status (cancelada conta), não se aplica a operações de reserva/adicionar reserva pelo detalhe/cancelar/transferir viagem, viagens antigas incompletas só são barradas no próximo salvamento (sem migration) — implementada na API (422 `data_ida_obrigatoria`/`data_volta_obrigatoria`/`reserva_obrigatoria`, contagem de reservas no banco na edição) e no front.
- **Bug corrigido:** expandir um `ReservationCard` sujava o form (marcava "Alterações não salvas" sem edição real).
- **Outros ajustes desta fase:** badge "Titular" voltou ao chip do passageiro (só leitura, sem controle de escolha); `useNovaViagem.ts` compactado para caber no limite de `max-lines` (350).
- **Pendências para Fase 2/3 (fora do escopo desta fase):** `ReservasTab` (tela de detalhe) ainda usa `business` em "+ Adicionar reserva" e "Cancelar viagem…" como `danger` sólido competindo visualmente com "Editar"; card "Viagens" solto entre cabeçalho e abas na lista; `FaixaResumo` quebra "Receita recebida" em telas estreitas.
- Backend `feat/redesign-viagem-f1` (base `cfb35a4`, T1 `07b0127`). Frontend `feat/redesign-viagem-f1` (base `dcd18c8`, T2 `017c0cb` · T3 `a33f03e` · T4 `dba3e2e` · T5 `5fe90fb` · T6 `58946fc`).
- **Mergeado em `develop` local 2026-09-14, sem push** (backend `da01978`, frontend `864d4eb`). Pós-merge, `fix/pendencias-redesign-f1` (2026-09-15): aviso "N campos precisam de atenção" só depois de tentar salvar; cabeçalho da página deixa de ser fixo em ≤1024 px; selo "calculado" do `MoneyInput` nunca sobrepõe o valor (some em campo < 220 px via container query) e "Total cobrado do cliente" mostra "Igual ao total da reserva"; linha financeira da reserva alinhada com helper/label de 2 linhas; data fixa no styleguide; **24 baselines do styleguide regeneradas em win32 e linux** (linux no container `mcr.microsoft.com/playwright:v1.63.0-noble`, sem API, igual ao CI). E2E `nova-viagem` + `viagens` 8/8 contra a API com a regra nova; 935 Vitest.
