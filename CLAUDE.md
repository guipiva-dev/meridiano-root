...
**Estado (2026-09-11):** Fase 1 (spec v2) ✓ · Fase 2 (esqueleto .NET, migrations 0001–0011) ✓ · Design v1 congelado (tag `design-v1-freeze`) ✓ · Fase 3 (3.0–3.6: front + todos os módulos, migrations 0012–0016; backend `main` `a163992`, frontend `main` `f6f0ceb`; 258 testes API + 30 domínio, 486 Vitest, Playwright `nova-viagem`/`viagens`/`cadastros`/`financeiro`/`operacao`; E2E 4 reservas 6,2 s @1280 / 7,7 s @1440) ✓ código — **fecha só com** teste de UX §7 e piloto §12 (humanos) · **Fase 4 (piloto) em andamento** — plano `docs/superpowers/plans/2026-09-11-fase-4-piloto.md`: onda 0 ✓ em `feat/fase-4` do backend (HEAD `b57ab27`, **sem merge/push**): migration 0017 (fecha PostgREST do Supabase), `scripts/bootstrap-agencia.sql`, `ci.yml` (GHCR + `az containerapp update`), `backup.yml` (pg_dump → R2); 260 API + 30 domínio. Ruling: **piloto local antes da nuvem** — imagem integrada, bootstrap, jobs e drill de restore validados no compose (2026-09-11; achados em `docs/BACKLOG.md` "Achados do piloto local"). Runbook `docs/deploy.md` escrito, não executado. Pendente: T5 (3 viagens da planilha + tempo humano, templates em `docs/piloto/`), T3 (nuvem), T4 (smoke em produção), T6 (fechamento). **Reviews externos 2026-09-11 aplicados** (backend `3fc6e72`, frontend `714305a`; itens rejeitados com motivo em `docs/BACKLOG.md` "Reviews externos"): migration 0018 (`senha_alterada_em`, `data_protection_key`), busca de clientes por visibilidade, cursor composto da auditoria, expurgo de soft-deleted, respostas stale no front, invalidação de listas, ErrorBoundary, timeout HTTP; 265 API + 30 domínio, 498 Vitest. Migrations 0001–0018.
