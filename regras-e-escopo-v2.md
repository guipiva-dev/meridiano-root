...
### 4.4 Conciliação
Reserva está **conciliada** quando `Σ (recebimento_operadora + estorno_operadora) ≥ valor_esperado_operadora`, ou quando `conciliacao_encerrada = true` com `divergencia_motivo` (operadora pagou menos e não vai pagar o resto). Reserva com esperado = 0 não entra na conciliação. Exceção: reserva **cancelada** com recebido > esperado (comissão recebida e não mantida) aparece como divergência `estorno_pendente` até o lançamento de `estorno_operadora` ou o encerramento com motivo.
